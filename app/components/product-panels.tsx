import { AlertTriangle, Check, CheckCircle2, ChevronRight, Copy, FileCheck2, FileText, GitCompareArrows, History, Laptop, LayoutTemplate, ListChecks, Monitor, MoreHorizontal, Plus, RotateCcw, Save, Smartphone, Sparkles, Tablet, Trash2 } from "lucide-react";
...
function DeliveryCheckPanel({ state, dispatch, canEdit, requirement }: Props) {
  ...
  const canGenerate = state.prototypeStatus === "confirmed" && state.prdStatus === "confirmed" && state.conflicts.every((item) => item.status === "resolved");
  const artifacts = [["原型", state.prototypeStatus, state.prototypeVersions.at(-1)?.version], ["PRD", state.prdStatus, state.prdVersions.at(-1)?.version], ["Spec", state.specStatus, state.specStatus === "confirmed" ? "V1" : "待生成"], ["验收标准", state.acceptanceStatus, state.acceptanceStatus === "confirmed" ? "12 条" : "待生成"]] as const;
  return <section className="product-panel delivery-check-panel">
    <ProductContext requirement={requirement} state={state} />
    <div className="product-panel-heading"><div><span>交付检查</span><h3>产品内容是否足够支持下游工作？</h3><p>完整性用于提示风险，不是研发和测试的强制门禁。</p></div><strong>{readiness.complete ? "完整" : `${4 - readiness.missing.length}/4`}</strong></div>
    ...
    <div className="spec-generate-card"><div><FileCheck2 size={20} /><span><b>Spec 与验收标准</b><small>{canGenerate ? "内容一致，可以基于已确认原型和 PRD 生成" : "请先确认原型、PRD 并处理内容冲突"}</small></span></div><button disabled={!canEdit || !canGenerate || state.specStatus === "confirmed"} onClick={() => dispatch({ type: "generate-spec" })} type="button">{state.specStatus === "confirmed" ? "已生成 Spec V1" : "生成 Spec 与验收标准"}</button></div>
    <div className={`product-completion-card ${state.productStatus}`}><div><Check size={19} /><span><b>{state.productStatus === "adjusting" ? "产品调整中" : "产品已标记完成"}</b><small>{state.productStatus === "complete-incomplete" ? `仍缺少${readiness.missing.join("、")}，同项目研发与测试已可见` : state.productStatus === "complete" ? "需求包完整，同项目研发与测试已可见" : "可以随时完成，缺失项只作为风险提示"}</small></span></div><button disabled={!canEdit} onClick={() => dispatch({ type: state.productStatus === "adjusting" ? "mark-product-complete" : "reopen-product" })} type="button">{state.productStatus === "adjusting" ? "标记需求完成" : "重新进入产品调整"}</button></div>
  </section>;
}
